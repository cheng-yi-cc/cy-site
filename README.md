...
- 自动化：GitHub Actions —— 推送即构建，并每 6 小时同步一次项目资料；构建完成后自动部署

## 工作原理

```
你把 .docx / .md 放进 articles/ 并推送，或定时同步被触发
        ↓ GitHub Actions 自动触发
...
网站仓库推送后会立即抓取一次；即使网站代码没有变化，GitHub Actions 也会每 6 小时
重新构建并抓取每个仓库的星标数、语言和 README。README 中的相对链接会改写为
GitHub 链接，图片会转存到本站。因此项目仓库的 README 更新通常会在 6 小时内上线，
无需手动重新部署网站。

定时任务约在北京时间 02:23、08:23、14:23、20:23 触发，GitHub 繁忙时可能稍有延迟。
GitHub 会在公开仓库连续 60 天没有活动时自动停用定时工作流；如果长期没有修改本站，
可在仓库的 Actions 页面重新启用“构建并部署网站”，或手动运行一次完成同步。
...
用新的正方形、高对比度线稿替换 `src/assets/home-particle-avatar.png`，保持文件名不变即可。背景脚本会自动采样深色线条生成粒子，并以 2.5 秒松散聚合动画入场；完成后桌面端保留鼠标互动和轻微漂移，移动端定格。替换后运行 `npm run build` 检查资源能否正常打包。粒子数量和刷新率已经针对 Chrome 做过优化，如需提高，应先在高刷新率屏幕上进行性能测试。
